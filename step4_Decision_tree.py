"""Step 4 - Train and evaluate a decision-tree treatment-outcome model."""
...
# IDs and other outcome columns should not be used to predict treatment outcome.
...
# Convert date text into useful calendar features before encoding categorical data.
...
for split_name, split_features, split_target in [
    ...
    split_predictions = model.predict(split_features)
    split_probabilities = model.predict_proba(split_features)[:, 1]
    print(
...
importance = pd.Series(
    model.named_steps["classifier"].feature_importances_, index=feature_names
